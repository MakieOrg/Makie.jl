...
function sample_bytes(screen, n)
    ...
    GC.enable(false)
    bytes = try
        # TWO windows, and the first is thrown away. Stopping the collector does
        # not stop a Vector from growing: a render SUBMITS and does not wait, so
        # the queue's in-flight list lengthens until the device catches up and
        # reallocates on the way — 3488 B in the FIRST sample of a window and 0
        # in all nineteen after it, then 0 in every sample of the next window.
        # A list still reaching its working depth is not the steady state either.
