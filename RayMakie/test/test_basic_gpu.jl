# Test script for basic GPU rendering - no volumes
# julia --check-bounds=yes test_basic_gpu.jl
...
println("Testing basic rendering with ROCArray (no volumes)...")
println("AMDGPU functional: ", AMDGPU.functional())

# Test with ROCArray backend - NO VOLUMES
let
    lights = [
    ...
    mesh!(ax, Sphere(Point3f(0, 0, 0.25), 0.25f0), material=diffuse_red)
