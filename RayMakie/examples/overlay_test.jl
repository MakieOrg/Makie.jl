# Overlay Test - Compare RayMakie overlay with GLMakie
# Tests scatter markers and lines on top of a ray-traced surface

using Revise, GLMakie, RayMakie, FileIO, ImageShow
...
begin
    ...
    RayMakie.interactive_window(fig; backend=AMDGPU.ROCBackend())
    display(fig; backend=GLMakie)
end

using Random
colorbuffer(surface(rand(5, 5)); backend=RayMakie)
