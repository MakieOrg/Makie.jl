...
@testset "grouped bar: stack" begin
	...
	from, to = stack_grouped_from_to(grp_stack1, y1, (; x1, grp_dodge1))
	from1 = [0.0, 2.0,  0.0, -3.0]
	to1   = [2.0, 5.0, -3.0, -5.0]
	@test from == from1
	@test to   == to1

	from, to = stack_grouped_from_to(grp_stack2, y2, (; x2, grp_dodge2))
	...
	from, to = stack_grouped_from_to(grp_stack, y, (; x, grp_dodge))
	@test from == from_test
    @test to == to_test
end