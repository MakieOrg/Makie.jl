    ...
    @testset "rtick rotations" begin
        ...
        for i in 1:4
            ...
            for j in 1:3
                ...
                @test rticklabelplot.plots[1].offset[] ≈ 10.0f0 * Vec2f(c, s)
                @test rticklabelplot.align[] ≈ align
                @test isapprox(mod(rticklabelplot.rotation[], -pi .. pi), (-pi / 4 + 0.01, 0, pi / 4 - 0.01)[j], atol = 1.0e-3)
            end
        end
        ...
        @test rticklabelplot.plots[1].offset[] ≈ 10.0f0 * Vec2f(c, s)
        @test rticklabelplot.align[] ≈ Point2f(0.5 - 0.5scale * c, 0.5 - 0.5scale * s)
        @test rticklabelplot.rotation[] ≈ 0.0f0
