...
@testset "Lighting" begin
    @testset "Shading default" begin
        ...
        @test Makie.get_shading_default(lights) === :none
        push!(lights, AmbientLight(RGBf(0.1, 0.1, 0.1)))
        @test Makie.get_shading_default(lights) === :fast
        push!(lights, PointLight(RGBf(0.1, 0.1, 0.1), Point3f(0)))
        @test Makie.get_shading_default(lights) === :fast
        push!(lights, PointLight(RGBf(0.1, 0.1, 0.1), Point3f(0)))
        @test Makie.get_shading_default(lights) === :verbose
        ...
        @test Makie.get_shading_default(lights) === :none # only affects RPRMakie so skipped here
