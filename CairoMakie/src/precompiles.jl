...
function draw_plot(scene, screen, Typ, args...; kw...)
    ...
    if isempty(plot.plots)
        CairoMakie.draw_atomic(scene, screen, TypedPlot{Typ}(plot))
    else
        p = plot.plots[1]
        if isempty(p.plots)
            CairoMakie.draw_atomic(scene, screen, TypedPlot{Typ}(p))
        else
            CairoMakie.draw_atomic(scene, screen, TypedPlot{Typ}(p.plots[1]))
        end
    end
end
...
function _precompile_()
    ...
    draw_plot(scene, screen, Makie.Text, ""; color=RGBA{Float32}(0.0f0,0.0f0,0.0f0,1.0f0), rotation=0.0f0, position=Point2f(400.0, 12.0), visible=true, font="dejavu sans book", markerspace=:data, align=(:center, :top), inspectable=false, textsize=16.0)
    ...
    draw_plot(scene, screen, Makie.Text, ""; color=RGBA{Float32}(0.0f0,0.0f0,0.0f0,1.0f0), rotation=1.5707964f0, position=Point2f(10.0, 300.0), visible=true, font="dejavu sans book", markerspace=:data, align=(:center, :bottom), inspectable=false, textsize=16.0)
    ...
    draw_plot(scene, screen, Makie.Makie.Text, Tuple{AbstractString, Point{2, Float32}}[("200", [11.0, 196.01993]), ("400", [11.0, 384.6459]), ("600", [11.0, 573.2719])]; color=RGBA{Float32}(0.0f0,0.0f0,0.0f0,1.0f0), rotation=0.0, font="dejavu sans book", visible=true, markerspace=:data, align=(:right, :center), inspectable=false, textsize=16.0)
    ...
    draw_plot(scene, screen, Makie.Text, ""; color=RGBA{Float32}(0.0f0,0.0f0,0.0f0,1.0f0), position=Point2f(400.0, 588.0), visible=true, font="dejavu sans book", markerspace=:data, align=(:center, :bottom), inspectable=false, textsize=16.0)
