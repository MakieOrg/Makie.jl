...
i.e. to initialize their compute graphs in bulk, potentially with nested
attributes. This makes the struct more complicated than just nested Dicts.

### Fields

#### `nesting::ComputePipeline.NestedSearchTree`

This field encodes the nesting of the attributes. It contains a
`Vector{Dict{Symbol, Int}}` which mirrors the nested dicts in a type stable way.
The elements each map a key to index which points to the next nesting layer if
it is > 0 or it points to a leaf attribute if < 0.

#### Per-Attribute fields

Each attribute in the struct has a merged key, default, docstring, default
expressions and a type. The first four are vectors indexed by the absolute of
the negative indices in `nesting`:
- `attr.merged_keys[i]`: `Symbol(join(keys, '.'))` where `keys` represents the
    keys needed to traverse the nested structure to the attribute. This is also
    used to refer to the attribute in compute graph, i.e. the attribute can be
    addressed by this name in a plot/block.
- `attr.defaults[i]`: the attribute value set in `@DocumentedAttributes` (etc.)
- `attr.leaf_docstring[i]`: the docstring defined for the attribute in
    `@DocumentedAttributes` or `nothing`
- `attr.default_expr[i]`: The expression used to define the attribute value

#### Attribute Types

For blocks, attributes can be defined with a type annotation. To reduce repeated
type inference, types are not saved per attribute. Instead each type is saved
once with the attribute indices that use that type:
- `attr.types` is a `Vector` of attribute types
- `attr.type_indices` contains a `Vector` of attribute indices for each type

To more easily lookup the type for an attribute index there also is:
- `attr.type_index`: Index into `attr.types` for each attribute index.

#### Other

- `attr.nested_docstring` contains a docstring or nothing for each positive
    index in `attr.nesting`, i.e. for each nesting layer. The base layer (1) is
    never documented
- `attr.merged_key_to_index` a `Dict` mapping merged keys to attribute indices
- `attr.inherit` a `Vector` of attribute indices that inherit
"""
