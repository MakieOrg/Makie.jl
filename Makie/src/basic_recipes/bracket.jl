...
* `xs1, ys1, xs2, ys2` Defines brackets where each argument is a `Real` or `AbstractVector{<:Real}`
    setting the respective x or y coordinate of the start (1) or end point (2) of a bracket.
* `points1, points2` Defines brackets where each argument is a `VecTypes{2, <:Real}` (`Point`, `Vec`
    or `Tuple`) or `AbstractVector{<:VecTypes}` setting the (x, y) coordinate of a start point (1)
    or end point (2) of a bracket.
