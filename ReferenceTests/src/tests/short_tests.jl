...
@reference_test "colorbuffer for axis" begin
    ...
    f, ax5, pl = image(img1; axis = (; aspect = DataAspect()))
    ax6, pl = image(f[1, 2], img2; axis = (; aspect = DataAspect()))
    ax7, pl = image(f[2, 2], img3; axis = (; aspect = DataAspect()))
    ax8, pl = image(f[2, 1], img4; axis = (; aspect = DataAspect()))
    hidedecorations!(ax5)
    hidedecorations!(ax6)
    hidedecorations!(ax7)
    hidedecorations!(ax8)
