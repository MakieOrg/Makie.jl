...
@reference_test "Surface with NaN points" begin
    ...
    scatter!(a, m.position, color = isnan.(m.normal), depth_shift = -1.0f-3)
    wireframe!(a, m, depth_shift = -1.0f-3, color = :black)
    f
end
...
@reference_test "meshscatter marker conversions with model" begin
    ...
    axis_kwargs = (xticks = ticks, yticks = ticks, limits = 1.0e12 .+ (-1.2e6, 1.2e6, -1.2e6, 1.2e6))
    ax1 = Axis(fig[3, 1]; axis_kwargs...)
    p7 = meshscatter!(ax1, [Point2f(1.0e12)], marker = Circle(Point2f(0), 1.0f6); transform_marker = false, kwargs...)
    ax2 = Axis(fig[3, 2]; axis_kwargs...)
    p8 = mesh!(ax2, Circle(Makie.Point2d(1.0e12), 1.0e6); kwargs2...)
    ax3 = Axis(fig[3, 3]; axis_kwargs...)
    p9 = meshscatter!(ax3, [Point2f(1.0e12)], marker = Circle(Point2f(0), 1.0f6); transform_marker = true, kwargs...)
    transform!.((p7, p8, p9), 5.0e11, false)
