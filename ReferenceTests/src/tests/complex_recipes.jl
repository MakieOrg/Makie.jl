...
@recipe complex AllBlocks (positions,) begin
...
function Makie.plot!(cr::AllBlocks)
    ...
    ax3 = Axis3(cr[2, 2])
    p2 = scatter!(ax3, cr.positions; color=cr.scatter_color, markersize = cr.markersize)
    po = PolarAxis(cr[3, 2])
    hidedecorations!(po, grid = false)
    p3 = lines!(po, cr.positions; color=cr.line_color, linewidth = cr.linewidth, linestyle = cr.linestyle)
...
@reference_test "Complex Recipes" begin
    ...
    fig, ax, cr = with_theme(my_theme, linestyle = :dash) do
        allblocks(1:0.5:10, sin, markersize = 20)
    end
