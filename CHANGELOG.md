...
- Fixed `BoundsError` in CairoMakie when clip planes remove all points of a plot, and fixed `apply_transform(::Mat4, ::Plane3)` producing a `NaN`-distance plane that clips everything when the transform collapses the plane's normal direction (e.g. `Axis3` with `aspect = :data` and zero-width limits) [#XXXX](https://github.com/MakieOrg/Makie.jl/pull/XXXX).
