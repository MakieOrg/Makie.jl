...
- **breaking** `image(mat)` now renders upright by default (image-storage convention). The new `storage` attribute is a tuple `(d1, d2)` of directions (`:up`/`:down`/`:left`/`:right`, one vertical and one horizontal) describing how the two array dims run on the conceptually-oriented quad. Default `(:down, :right)` matches typical image storage; `(:right, :down)` is the legacy Makie convention. Freshly-created axes also get `yreversed = true` as a hint. The user matrix flows through to backends untouched — the layout is composed into the existing `uv_transform`.
