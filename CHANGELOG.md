...
- WGLMakie no longer blocks (up to 100s) when plots are inserted into or deleted from a displayed figure whose websocket isn't connected: static HTML exports, precompile workloads, or a tab that never finishes loading. Before the browser reports scene initialization nothing needs to be sent (the scene is serialized with its current state at that point); afterwards, operations on a disconnected session go through Bonito's message queue and are delivered on reconnect, with the JS side deferring operations that arrive before their target scene is initialized. When the connection is live, `insert!` still round trips, so the plot is guaranteed to exist in the browser when it returns.
