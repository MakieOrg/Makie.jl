...
- Added `strokewidth`, `strokecolor` and `strokeedges` attributes to `mesh`, which render edge strokes as part of the mesh surface itself. In CairoMakie, `mesh` now renders through a new software rasterizer with a per-plot depth buffer instead of Cairo triangle patches [#5727](https://github.com/MakieOrg/Makie.jl/pull/5727)
