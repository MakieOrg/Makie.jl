...
- RayMakie: a screen now takes `samples`, how many samples ONE finished frame costs, overriding the integrator's `samples_per_pixel`; `colorbuffer(screen; samples, clear)` overrides even that per read. A path tracer's frame is a running average, so the count belongs to the read: an interactive preview asks for one sample at a time with `clear = false` and converges while nothing moves, a bake asks for the whole budget in one call.
- RayMakie: the scene's background alpha is no longer dropped (`RGBA(r, g, b, 1f0)`), so a raytraced scene composited over another picture leaves the misses uncovered instead of replacing it — and a ray that escaped under an infinite light counts as a miss. `Hikari` writes a large finite depth rather than `Inf` once the scene has an environment map, sun or ambient, so the old `isinf` test found that nothing had escaped and every pixel came back opaque. Escaped rays keep the radiance an infinite light gave them (`sky`) instead of being painted over with the background.
- Fixed a `Tabs` header taking clicks while its scene is hidden or covered. `tab_at` answers from the tab rects alone, so a dismissed dialog left in the figure kept switching its own tab and consumed the click meant for the dialog on top of it. The header's handlers now guard on `receives_events`, as `Menu`, `Legend` and `Table` already do.
