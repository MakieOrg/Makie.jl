...
- Stopped CairoMakie drawing a uniformly coloured 2D mesh through a Cairo mesh pattern, which SVG has no way to express, so the mesh was rasterised into the output. Such a mesh is now filled with a plain source, making it a real path: an SVG of one went from 27.7 kB with an embedded raster to 7.6 kB of vector, an EPS from 17.4 kB to 7.9 kB, and a PDF from 22.0 kB to 2.1 kB. Gradient-coloured meshes still need the pattern; those are already vector in PDF, as a `/ShadingType 7` shading, but cannot be in SVG.
- Filled a 2D mesh coloured with a `Makie.AbstractPattern` as a single path in CairoMakie, rather than one triangle at a time. Each triangle was anti-aliased on its own, so two triangles sharing an edge only covered about 75% of it between them and every interior edge showed as a seam.
