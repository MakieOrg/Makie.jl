"""
    bezier(x, y; npoints = 30, kw_args...)
    bezier(positions::Point2f; npoints = 30, kw_args...)

Plot a Bezier curve between each point in `positions`, or each pair of values in `zip(x, y)`.
Takes any input that you can pass to `lines`.

The `npoints` attribute controls how many points of the Bezier curve are sampled
between each pair of input points.  If you pass `color` to the plot, then it is
assumed to be per input point.
"""
