...
function plot!(plot::T) where T <: Union{Contour, Contour3d}
    ...
    lift(projection_obs(plot), labels, labelcolor, labelformatter, lev_pos_col) do _,
        ...
        for (lev, (p1, p2, p3), color) in lev_pos_col
            rot_from_horz::Float32 = projected_angle(plot, p1, p3)
    ...
    bboxes = lift(labels, texts.text; ignore_equal_values=true) do labels, _
        ...
        return broadcast(texts.plots[1][1].val, texts.positions.val, texts.rotation.val) do gc, pt, rot
            ...
            Rect2f(boundingbox(gc, project(plot, pt), to_rotation(rot)))
        end
    end
    ...
    masked_lines = lift(labels, bboxes, points) do labels, bboxes, segments
        ...
        for (i, p) in enumerate(segments)
            ...
            elseif project_to_pixel(plot, p) in bb
                masked[i] = nan
                for dir in (-1, +1)
                    j = i
                    while true
                        ...
                        project_to_pixel(plot, segments[j]) in bb || break
                        masked[j] = nan
                    end
                end
            end
