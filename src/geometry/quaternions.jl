...
Random.rand(mt::AbstractRNG, ::Random.SamplerType{Quaternion}) =
    rand(mt, Quaternion{Float64})
Random.rand(mt::AbstractRNG, ::Random.SamplerType{Quaternion{T}}) where {T} =
...
function qrotation(axis::Vec3, theta::Number)
    u = normalize(axis)
    s = sin(theta / 2)
...
function Base.:(*)(quat::Quaternion, vec::Vec{2,T}) where {T}
    x3 = quat * Vec(vec[1], vec[2], T(0))
    Vec(x3[1], x3[2])
end

function Base.:(*)(quat::Quaternion{T}, vec::Vec3) where {T}
    num = quat[1] * T(2)
    num2 = quat[2] * T(2)
    num3 = quat[3] * T(2)
    ...
    return Vec3(
        (1.0f0 - (num5 + num6)) * vec[1] +
        (num7 - num12) * vec[2] +
        (num8 + num11) * vec[3],
        (num7 + num12) * vec[1] +
        (1.0f0 - (num4 + num6)) * vec[2] +
        (num9 - num10) * vec[3],
        (num8 - num11) * vec[1] +
        (num9 + num10) * vec[2] +
        (1.0f0 - (num4 + num5)) * vec[3],
    )
end
...
function orthogonal(v::Vec3)
    x, y, z = abs.(v)
    other = x < y ? (x < z ? unit(T, 1) : unit(T, 3)) : (y < z ? unit(T, 2) : unit(T, 3))
    return cross(v, other)
end

function rotation_between(u::Vec{3,T}, v::Vec{3,T}) where {T}
    k_cos_theta = dot(u, v)
