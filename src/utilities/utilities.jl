...
function extract_color_recursive(@nospecialize(plot))
    color = extract_color(attributes(plot))
    if color === nothing && !is_atomic_plot(plot)
        extracted_colors = unique(map(extract_color, collect_atomic_plots(plot)))
        if length(extracted_colors) == 1
            return extracted_colors[1]
        elseif !isempty(extracted_colors)
            if any(x-> x isa Dict, extracted_colors)
                filter!(x-> x isa Dict, extracted_colors)
                return only(extracted_colors)
            end
        else
            error("Multiple or no colors found: $(map(typeof, extracted_colors))")
        end
    end
    return color
end

function extract_color(attributes)
    if haskey(attributes, :calculated_colors)
        color = attributes[:calculated_colors]
        if color[] isa ColorMap
            result = Dict{Symbol, Any}()
            result[:colorrange] = color[].colorrange
            result[:colormap] = color[].colormap
            result[:highclip] = color[].highclip
            result[:lowclip] = color[].lowclip
            return result
        end
        return color
    else
        if haskey(attributes, :colormap) &&  haskey(attributes, :colorrange) && !(attributes.colorrange[] isa Automatic)
            if haskey(attributes, :color) && attributes[:color][] isa Union{Number,Cycled}
                return nothing
            end
            result = Dict{Symbol, Any}()
            result[:colorrange] = attributes[:colorrange]
            result[:colormap] = attributes[:colormap]
            haskey(attributes, :highclip) && (result[:highclip] = attributes[:highclip])
            haskey(attributes, :lowclip) && (result[:lowclip] = attributes[:lowclip])
            return result
        end
    end
    if haskey(attributes, :color) && attributes[:color][] isa Union{Colorant, AbstractArray{<: Colorant}}
