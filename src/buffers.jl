function LinesegmentBuffer(scene::Scene, ::Type{Point{N}} = Point{2}) where N
    linesegments!(scene, Point{N, Float32}[]; color = RGBAf0[], linewidth = Float32[])
end
...
function TextBuffer(scene, ::Type{Point{N}} = Point{2}) where N
    annotations!(
