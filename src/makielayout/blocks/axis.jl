...
function update_axis_camera(camera::Camera, t, lims, xrev::Bool, yrev::Bool)
    ...
    if all(isfinite, args)
        projection = orthographicprojection(args..., nearclip, farclip)
        set_proj_view!(camera, projection, Mat4f(I))
    else
...
function initialize_block!(ax::Axis; palette = nothing)
    ...
    xlims = Observable(xlimits(finallimits[]); ignore_equal_values=true)
    ylims = Observable(ylimits(finallimits[]); ignore_equal_values=true)
...
"""
function reset_limits!(ax::Axis; xauto=true, yauto=true, zauto=true)
    ...
    elseif ax isa Axis3
        ax.targetlimits[] = Rect3f(Vec3f(xlims[1], ylims[1], zlims[1]),
