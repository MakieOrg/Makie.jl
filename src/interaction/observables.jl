...
Create a new `w::ChangeObservable` by applying `f` to the _values_ of all observables
...
ChangeObservable{Int64} with 0 listeners. Value:
2
...
function lift(f, o1::AbstractObservable, rest...; change_only = false, kw...)
    ...
    typ = typeof(init)
    obstype = change_only ? ChangeObservable : Observable
    result = obstype{typ}(init)
