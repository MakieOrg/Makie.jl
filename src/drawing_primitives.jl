...
function draw_atomic(screen::GLScreen, scene::Scene, x::Mesh)
    robj = cached_robj!(screen, scene, x) do gl_attributes
        ...
        elseif to_value(color) isa AbstractPlotting.AbstractPattern
            img = lift(x -> el32convert(AbstractPlotting.image(x)), color)
...
function draw_atomic(screen::GLScreen, scene::Scene, x::Surface)
    robj = cached_robj!(screen, scene, x) do gl_attributes
        ...
        elseif to_value(color) isa AbstractPlotting.AbstractPattern
            pattern_img = lift(x -> el32convert(AbstractPlotting.image(x)), color)
            img = ShaderAbstractions.Sampler(pattern_img, x_repeat=:repeat)
            haskey(gl_attributes, :fetch_pixel) || (gl_attributes[:fetch_pixel] = true)
