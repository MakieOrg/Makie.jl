...
function cheatsheet_3d(rn, testimage)
    ...
    heatmap!(axs3, 1:10, 1:10, rn; colormap=:tableau_sunset_sunrise)
    ...
    text!(axs5, Point2f(0), text=rich("Say", rich(" something", color=:red), "\nfunny !", color=:black, font=:bold);
        rotation=π / 4, align=(:center, :center))
    ...
    z = Makie.peaks()
    x = LinRange(-3, 3, size(z, 1))
    wireframe!(axs[8], x, x, z, transparency=true, color=:dodgerblue)
    axs[8].title = "wireframe(x,y,z)"

    contour3d!(axs[9], x, x, z, transparency=true,
        colormap=:diverging_bkr_55_10_c35_n256, levels=14)
    axs[9].title = "contour3d(x,y,z)"

    contourf!(axs[10], x, x, z; levels=14,
...
function cheatsheet_2d(testimage)
    ...
    spy!(axs[13], 0 .. 1, 0 .. 1, sprand(10, 10, 0.05);
