...
convert_attribute(::Type, ::Key, x) = x

function convert_attribute(::T, x::Any, k::Key) where T <: AbstractPlot
    return convert_attribute(T, x, k)
end

function convert_attribute(::Type{T}, x::Any, k::Key) where T <: AbstractPlot
    return convert_attribute(x, k)
end
...
el32convert(x::Observable) = map(el32convert, x)
...
convert_attribute(c, ::key"markersize") = to_2d_scale(c)
