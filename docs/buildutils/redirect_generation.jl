function generate_redirects(rules; dry_run = true)
    for basedir in readdir(joinpath(@__DIR__, "..", "build"), join = true)
        isdir(basedir) || continue
        htmlpaths = cd(basedir) do
            collect(
                Iterators.flatmap(walkdir(".")) do (root, dirs, files)
        ...
        for (redirect_file, existing_file) in redirects_dict
            write_redirection_html(basedir, redirect_file, existing_file; dry_run)
        end
    end
...
function write_redirection_html(basedir, redirect_file, existing_file; dry_run)
