...
color_tint = Makie.Stage(:OklabToLinearRGB,
...
function GLMakie.construct(::Val{:OklabToLinearRGB}, screen, framebuffer, inputs, parent)
    ...
    void main(void) {
        ...
        // Convert from Oklab-normed to linear RGB
        // First convert to Oklab (denormalize a and b channels)
        vec3 oklab = vec3(c.r, (c.g - 0.5) * 0.8, (c.b - 0.5) * 0.8);
        
        // Convert Oklab to linear RGB using the standard matrix
        // vec3 linear_rgb = mat3(
        //     1.0,  1.0,  1.0,
        ...
        fragment_color = vec4(vec3(c.r, 0, 0), c.a);
    }
    """
...
function GLMakie.run_step(screen, glscene, step::GLMakie.RenderPass{:OklabToLinearRGB})
...
scatter(rand(1000), rand(1000); color = rand(1000))