...
```
f, ax, pl = scatterlines(args...; kw...) # return a new figure, axis, and plot
   ax, pl = scatterlines(f[row, col], args...; kw...) # creates an axis in a subfigure grid position
       pl = scatterlines!(ax::Union{Scene, AbstractAxis}, args...; kw...) # Creates a plot in the given axis or scene.
SpecApi.Scatterlines(args...; kw...) # Creates a SpecApi plot, which can be used in `S.Axis(plots=[plot])`.
```
...
For detailed conversion information, see `Makie.conversion_docs(Scatterlines)`.
...
See the [online documentation](https://docs.makie.org/stable/reference/plots/scatterlines) for rendered examples.
