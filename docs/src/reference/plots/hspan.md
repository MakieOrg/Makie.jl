...
```
f, ax, pl = hspan(args...; kw...) # return a new figure, axis, and plot
   ax, pl = hspan(f[row, col], args...; kw...) # creates an axis in a subfigure grid position
       pl = hspan!(ax::Union{Scene, AbstractAxis}, args...; kw...) # Creates a plot in the given axis or scene.
SpecApi.Hspan(args...; kw...) # Creates a SpecApi plot, which can be used in `S.Axis(plots=[plot])`.
```
...
See the [online documentation](https://docs.makie.org/stable/reference/plots/hspan) for rendered examples.
