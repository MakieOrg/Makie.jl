...
```
f, ax, pl = stairs(args...; kw...) # return a new figure, axis, and plot
   ax, pl = stairs(f[row, col], args...; kw...) # creates an axis in a subfigure grid position
       pl = stairs!(ax::Union{Scene, AbstractAxis}, args...; kw...) # Creates a plot in the given axis or scene.
SpecApi.Stairs(args...; kw...) # Creates a SpecApi plot, which can be used in `S.Axis(plots=[plot])`.
```
...
For detailed conversion information, see `Makie.conversion_docs(Stairs)`.
...
See the [online documentation](https://docs.makie.org/stable/reference/plots/stairs) for rendered examples.
