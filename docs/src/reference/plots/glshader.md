...
begin
    GLMakie.closeall()
    s = Scene()
    glshader!(s, Rect2f(-1, -1, 2, 2), read(joinpath(@__DIR__, "raytracing.frag"), String))
    scren = display(s)
end

begin
    f, ax, pl = glshader(Rect2f(-1, -1, 2, 2), read(joinpath(@__DIR__, "raytracing.frag"), String))
    foreach(n->deregister_interaction!(ax, n), keys(interactions(ax)))
    f
end
glshader(Rect2f(-1, -1, 2, 2), read(joinpath(@__DIR__, "monster.frag"), String))
```
