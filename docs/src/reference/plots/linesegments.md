...
```
f, ax, pl = linesegments(args...; kw...) # return a new figure, axis, and plot
   ax, pl = linesegments(f[row, col], args...; kw...) # creates an axis in a subfigure grid position
       pl = linesegments!(ax::Union{Scene, AbstractAxis}, args...; kw...) # Creates a plot in the given axis or scene.
SpecApi.Linesegments(args...; kw...) # Creates a SpecApi plot, which can be used in `S.Axis(plots=[plot])`.
```
...
For detailed conversion information, see `Makie.conversion_docs(Linesegments)`.
...
See the [online documentation](https://docs.makie.org/stable/reference/plots/linesegments) for rendered examples.
