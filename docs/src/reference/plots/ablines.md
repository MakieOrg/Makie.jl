...
```
f, ax, pl = ablines(args...; kw...) # return a new figure, axis, and plot
   ax, pl = ablines(f[row, col], args...; kw...) # creates an axis in a subfigure grid position
       pl = ablines!(ax::Union{Scene, AbstractAxis}, args...; kw...) # Creates a plot in the given axis or scene.
SpecApi.Ablines(args...; kw...) # Creates a SpecApi plot, which can be used in `S.Axis(plots=[plot])`.
```
...
See the [online documentation](https://docs.makie.org/stable/reference/plots/ablines) for rendered examples.
