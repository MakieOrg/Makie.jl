...
```
f, ax, pl = vspan(args...; kw...) # return a new figure, axis, and plot
   ax, pl = vspan(f[row, col], args...; kw...) # creates an axis in a subfigure grid position
       pl = vspan!(ax::Union{Scene, AbstractAxis}, args...; kw...) # Creates a plot in the given axis or scene.
SpecApi.Vspan(args...; kw...) # Creates a SpecApi plot, which can be used in `S.Axis(plots=[plot])`.
```
...
See the [online documentation](https://docs.makie.org/stable/reference/plots/vspan) for rendered examples.
