...
```
f, ax, pl = wireframe(args...; kw...) # return a new figure, axis, and plot
   ax, pl = wireframe(f[row, col], args...; kw...) # creates an axis in a subfigure grid position
       pl = wireframe!(ax::Union{Scene, AbstractAxis}, args...; kw...) # Creates a plot in the given axis or scene.
SpecApi.Wireframe(args...; kw...) # Creates a SpecApi plot, which can be used in `S.Axis(plots=[plot])`.
```
...
For detailed conversion information, see `Makie.conversion_docs(Wireframe)`.
...
See the [online documentation](https://docs.makie.org/stable/reference/plots/wireframe) for rendered examples.
