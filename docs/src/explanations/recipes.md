...
You can control which kind of axis is used for the plot by defining one of

```julia
Makie.preferred_axis_type(::MyPlot) = Makie.LScene
Makie.args_preferred_axis(::MyPlot) =  Makie.LScene
Makie.args_preferred_axis(::MyPlot, x, y, z) =  Makie.LScene
Makie.args_preferred_axis(::Type{<: MyPlot}, x, y, z) =  Makie.LScene
Makie.args_preferred_axis(::Type{<: MyPlot}) =  Makie.LScene
Makie.args_preferred_axis(x, y, z) =  Makie.LScene
```
...
They will be overwritten by attributes passed to a plot explicitly with `axis = (attr = val, ...)`.
Note that both interfaces only act when an axis is generated by a plot call, i.e. when the non-mutating `plot()` is used.
