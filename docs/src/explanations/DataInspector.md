...
The first is a trace of parent plots from the primitive up to the (user created) recipe plot.
...
Accessors are generated by methods of `get_accessor(plot, index, plot_stack)` where the `plot` is the root parent of the full plot stack, `index` is the index returned by `pick_sorted` and `plot_stack` is the plot stack without the root parent.
...
The plot visualizes discrete errors using `linesegments` for the bars and `scatter` for the whiskers.
...
We need to implement a new method to grab the correct ones.
...
function get_tooltip_position(element::PlotElement{<:Errorbars})
	...
	#	get_plot(element).val_low_high[][element.index]
...
This requires `constructor_indicator_plot(inspector, PlotType)` to exist.
