...
This is done with the `update!()` function, `setproperty!()` or `setindex!()`:

```julia
# Update a single value
graph.input1 = 1
graph[:input2] = 2
update!(graph, input3 = 3)

# update multiple inputs together (synchronized)
update!(graph, input1 = 5, input2 = 1)
# update!() also allows:
# Pairs:   update!(graph, :input1 => 5, :input2 => 1)
# Vectors: update!(graph, [:input1 => 5, :input2 => 1])
# Dicts:   update!(graph, Dict(:input1 => 5, :input2 => 1))
```
...
!!! danger
    Updating a node that is connected to an edge from within that edges callback is undefined behavior.
    It may cause the update to get lost, state to be corrupted or graph resolution to deadlock.
    To be safe, avoid calling `update!()`, `graph.x = ...`, `graph[:x] = ...`, etc. from inside a `map!()` or `register_computation!()` callback.
    Also avoid updating Observables that feed back into the graph from within a callback.
    Instead, rely on Observables generated by the compute graph to do these updates.
    using `map(val -> update!(graph, x = ...), graph.output)` or `on(...)` is safe, for example.

!!! note
    You can also update non-input nodes with `graph.node[] = value`.
    This behaves a bit differently from input updates which is why it uses the `[]` syntax.
    If the parent edge of the node was clean before the update, `value` will be the new value of node.
    From the outside, this case behaves like updating an input.
    Technically this breaks the promise that a nodes value is given by its parent edge and its inputs.
    E.g. if `graph.b[] = 10` is set with `map!(x -> x+1, graph, :a, :b)`, `graph.b[]` will no longer be `graph.a[] + 1`, but just `10`.
    If the parent edge was dirty before the update, the pending resolve will still happen and overwrite the value.
    In this case `graph.a[] + 1` will continue to be the result.
