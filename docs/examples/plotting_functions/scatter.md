...
Currently, such glyphs are picked from the `TeX Gyre Heros Regular` font, because it offers a wide range of symbols.
...
!!! note
    The scatter markers have the same sizes that the glyphs in TeX Gyre Heros Regular have. This means that they are not matched in size or area. Currently, Makie does not have the option to use area matched markers, and sometimes manual adjustment might be necessary to achieve a good visual result.
