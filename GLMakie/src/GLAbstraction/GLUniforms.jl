...
gluniform(location::Integer, target::Integer, t::AbstractObservable) = gluniform(GLint(location), GLint(target), to_value(t))
...
function gluniform(loc::Integer, x::AbstractObservable{T}) where T
...
toglsltype_string(t::AbstractObservable) = toglsltype_string(to_value(t))
...
function glsl_variable_access(keystring, s::AbstractObservable)
...
gl_convert(x::AbstractObservable{T}) where {T <: GeometryBasics.Mesh} = gl_promote(T)(x)
...
gl_convert(s::AbstractObservable{T}) where {T <: NATIVE_TYPES} = s
gl_convert(s::AbstractObservable{T}) where T = const_lift(gl_convert, s)
...
gl_convert(::Type{<: GPUArray}, a::AbstractObservable{<: StaticVector}) = gl_convert(a)

function gl_convert(::Type{T}, a::AbstractObservable{<: AbstractArray{X, N}}; kw_args...) where {T <: GPUArray, X, N}
    TGL = gl_promote(X)
