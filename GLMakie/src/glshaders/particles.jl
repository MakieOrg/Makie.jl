...
function vec2quaternion(r::StaticVector{2})
    return vec2quaternion(Vec3f(r[1], r[2], 0))
end
function vec2quaternion(rotation::StaticVector{3})
    return Makie.rotation_between(Vec3f(0, 0, 1), Vec3f(rotation))
end
    ...
    if to_value(pop!(data, :depthsorting, false))
        data[:indices] = map(
            ...
            depth_vals = map(pos) do p
                p4d = T * to_ndim(Point4f, to_ndim(Point3f, p, 0.0f0), 1.0f0)
                p4d[3] / p4d[4]
            end
