...
directly but be generated by [`add_input!`](@ref) and [`register_computation!`](@ref).

A `Computed` can be accessed from the graph by `graph[:name]` or `graph.name`
where `:name` is the name the node was created with. To get the value contained
in a node the dereference operator should be used, i.e. `graph[:name][]`. This
will trigger the node being "resolved", meaning that any pending computations
are executed before returning a value.
"""
...
be updated by `update!(graph, input_name = new_value)` or `graph.input_name = new_value`
to correctly update the state of the compute graph.
"""
...
outputs they write to can be added with [`register_computation!`](@ref).

To update an input, `update!(graph, input_name = new_value)` and
`graph.input_name = new_value` can be used. To get an up-to-date value
from an output use `graph.output_name[]` or `graph[:output_name][]`.
"""
struct ComputeGraph
...
pairs represented as keyword arguments. The `key` is used as the name of the
input and the `value` acts as the new value.
"""
...
Adds a new `Input` referred to by `name` to the `compute_graph` holding the
given `value`. Optionally a `callback = (value) -> transformed_value` can be
added to apply a conversion or transformation whenever the value is updated.

The `Input` acts as an access point to the compute graph. It can be updated with
`graph.name = new_value` or `update!(graph, name = new_value)`. Every `Input` is
backed by a `Computed`, i.e. a node in the compute graph with the same `name`
...
This does not create an `Input` and thus makes `name` only updatable through
updates of `node`.
"""
...
Registers the given `callback::Function` as a computation to run on inputs
referred to by `input_names::Vector{Symbol}` and writing to outputs referred to
by `output_names::Vector{Symbol}`. Output nodes that do not yet exists will be
created by this function. Output nodes that do exist must not yet have a parent
computation. Input nodes must already exist.

The callback should accept 3 arguments, a `NamedTuple` of inputs, a `NamedTuple`
marking which inputs changed and a `Tuple` of previous results. The output should
be either a tuple with one value per output or `nothing` if no new values are
present. `nothing` can also be returned for individual outputs.
