# Test script for Smoke volume - run with bash to avoid crashing REPL
# julia --check-bounds=yes test_smoke_volume.jl
...
let
    ...
    ax = Scene(; size=(1000, 1000), lights=lights, ambient=RGBf(0, 0, 0))
    cam3d!(ax)
    ...
    integrator = Hikari.VolPath(samples=30, max_depth=20, filter=Hikari.GaussianFilter())
