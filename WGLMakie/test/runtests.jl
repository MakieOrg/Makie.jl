...
@testset "memory leaks" begin
    ...
    @test session_size / 10^6 < 10
    @test texture_atlas_size < 10
    ...
    wgl_scenes = run(edisplay.window, "Object.keys(WGL.scene_cache)")
    wgl_plots = run(edisplay.window, "Object.keys(WGL.plot_cache)")
    @test isempty(wgl_scenes)
    ...
    # @test keys(js_objects) == Set([WGLMakie.TEXTURE_ATLAS.value.id])
end
