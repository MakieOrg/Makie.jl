...
# @reference_test "HTML Widgets - comprehensive layout" begin
begin
    ...
    # Row 5 - Buttons
    btn_add = Makie.Button(fig[5, 1], label = "Add Point", tellwidth = false)
    btn_clear = Makie.Button(fig[5, 2:3], label = "Clear Points", tellwidth = false)
    ...
    Label(fig[6, 3], "Function:", halign = :right, tellwidth = false)
    funcs = [sin, cos, tan]
    menu_func = Makie.Menu(fig[6, 4],
    ...
    Label(fig[7, 3], "Number:", halign = :right, tellwidth = false)
    textbox_num = Makie.Textbox(fig[7, 4], validator = Float64, stored_string = "3.14", tellwidth = false)
    ...
    modified_ys = lift(menu_func.selection, textbox_num.stored_string) do f, numstr
        ...
        multiplier .* f.(modified_xs)
    end
    lines!(ax2, modified_xs, modified_ys, color = :orange, linewidth = 2)
    ...
    # Update sliders using set_close_to!
    set_close_to!(sl_freq, 3.5)
    set_close_to!(sl_amp, 2.0)
    set_close_to!(sl_phase, π)
    set_close_to!(sl_vertical, 7.5)

    # Update menus by setting i_selected
    menu_cmap.i_selected[] = 3  # Select "inferno"
    menu_func.i_selected[] = 2  # Select "Cosine"

    # Update textboxes by setting stored_string
    Makie.set!(textbox_text, "Updated!")
    Makie.set!(textbox_num, "2.71")

    # Trigger button clicks
    btn_add.clicks[] = btn_add.clicks[] + 3  # Simulate 3 clicks

    # Add some points to scatter plot using the button's intended behavior
    # (simulating what would happen with actual button clicks)
    push!(points[], Point2f(sl_vertical.value[], 3))
    push!(points[], Point2f(sl_vertical.value[], 5))
    push!(points[], Point2f(sl_vertical.value[], 7))
    notify(points)

    fig
end
WGLMakie.activate!(;
    use_html_widgets=true,
