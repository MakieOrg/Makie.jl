...
Most options that are available today lack one or more of the following attributes: High performance for the responsive rendering of large datasets, interactive visualizations that can be updated live using dynamic languages, the ability to extend the plotting pipeline to handle user-defined data structures and types, implementations of both 2D and 3D rendering, and the power to construct complex figures without having to tweak subplot positions after the fact in image editing software.
